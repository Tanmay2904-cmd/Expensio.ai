...
const UsersPage = () => {
  ...
  const handleEdit = (user) => { setEditing(user); setOpen(true); };
  const handleDelete = async (id) => { await axiosInstance.delete(`/api/users/${id}`); fetchUsers(); };
  const handleSubmit = async (form) => {
    if (editing) await axiosInstance.put(`/api/users/${editing.id}`, { ...editing, ...form });
    else await axiosInstance.post('/api/users', form);
    setOpen(false);
    fetchUsers();
  };
